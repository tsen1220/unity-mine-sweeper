...
public class sixteenBoxScript : MonoBehaviour
{
    public bool mine;
    ...
    private int OriginXpos = -2;
    private int OriginYpos = -2;

    private float xMargin = 0.32f;
    private float yMargin = 0.32f;

    private Queue<int> breadthSearch = new Queue<int>();

    private sixteenBoxProduction BoxProduction;
    ...
    private void Update()
    {
        if (isFinished())
        {
            ...
            {
                Element.GetComponent<sixteenBoxScript>().enabled = false;
    ...
    private void showAllMine()
    {
        ...
        foreach (GameObject Element in boxProduction)
        {
            if (Element.GetComponent<sixteenBoxScript>().mine)
            {
    ...
    private void searchField()
    {
        ...
        if (!minePosition(X, Y))
        {
            ...
            foreach (GameObject Element in boxProduction)
            {
                ...
                {
                    Element.GetComponent<sixteenBoxScript>().loadElement(adjacentMines(X, Y));
                }
            }
    ...
    private bool isFinished()
    {
        ...
        foreach (GameObject Element in boxProduction)
        {
            if (!Element.GetComponent<sixteenBoxScript>().mine && Element.GetComponent<SpriteRenderer>().sprite.texture.name == "box")
            {
                return false;
            }
        }
